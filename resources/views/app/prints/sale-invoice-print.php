        ...
        <table class="table-print w100 color-print center mt15">
            ...
            <tbody>
                ...
                if (!empty($invoice_data) && is_array($invoice_data)) {
                    $number = 1;
                    $remaining_amount = $sale_invoice_print['total_amount'] - ($sale_invoice_print['paid_amount'] + $sale_invoice_print['discount']);
                ...
                ?>
                        <tr>
                            ...
                            <td class="w80"><?= $this->to_farsi_number(number_format($unitPrices['sell'])) ?></td>
            ...
            <table class="table-print w100 color-print center mt5">
                ...
                <tbody>
                    <tr class="fs15">
                        ...
                        <td class="bold">مانده از این فاکتور: <span><?= $this->to_farsi_number(number_format($remaining_amount)) ?> <?= _afghani ?></span></td>
                        <td class="bold">مجموع بدهی</td>
                        <td class="bold"><?= $this->to_farsi_number(number_format($remaining_amount + $sale_invoice_print['account_balance'])) ?> <?= _afghani ?></td>
                    </tr>
                </tbody>
            </table>
        <?php }
    ...
    <a href="<?= url('add-sale') ?>" class="back-sale cursor-p hover">برگشت به صفحه فروش</a>
    <?php
    $timestamp = $sale_invoice_print['date'];
    $discount = ($sale_invoice_print['discount']) ? '-' . $this->formatPrice($sale_invoice_print['discount']) : '';
    $omi = date('Ymd', $timestamp);
    $fa =  $sale_invoice_print['id'] . '-' . $omi . $discount;
    ?>
