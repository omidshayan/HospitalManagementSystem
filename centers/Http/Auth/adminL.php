...
class Login extends Auth
{
    ...
    {
        $db = new Database();
        if (isset($_COOKIE['er_em_co'])) {
            $remember_token = $db->select('SELECT * FROM `employees` WHERE remember_token = ?', [$_COOKIE['er_em_co']])->fetch();
            if ($remember_token != null && $remember_token > 0 && $remember_token['expire_remember_token'] == 1) {
                $_SESSION['er_em_id'] = $remember_token['id'];
                $_SESSION['er_em_name'] = $remember_token['employee_name'];
