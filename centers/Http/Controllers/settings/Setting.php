...
class Setting extends App
{
    ...
    public function changeStatusCompanyActive()
    {
        $this->middleware(true, true, 'general');

        $row = $this->db->select(
            'SELECT id, company FROM settings LIMIT 1'
        )->fetch();
        ...
        $_SESSION['settings']['company'] = $newStatus;

        $this->send_json_response(true, _success, $newStatus);
    }

    // change status description
    public function changeStatusDescriptionActive()
    {
        $this->middleware(true, true, 'general');

        $row = $this->db->select(
            'SELECT id, description FROM settings LIMIT 1'
        )->fetch();
        ...
        $newStatus = ($row['description'] == 1) ? 2 : 1;
        ...
        $_SESSION['settings']['description'] = $newStatus;

        $this->send_json_response(true, _success, $newStatus);
    }

    // change status drug type
    public function changeStatusDrugTypeActive()
    {
        $this->middleware(true, true, 'general');

        $row = $this->db->select(
            'SELECT id, drug_type FROM settings LIMIT 1'
        )->fetch();
        ...
        $_SESSION['settings']['drug_type'] = $newStatus;
